...
def generate_deterministic_schedule():
    ...
    # Create a date range for the entire year of 2020 with hourly frequency
    date_range = pd.date_range(start=f"{year}-01-01", end=f"{year}-12-31 23:00", freq="H")

    # Initialize the occupancy array with zeros
    full_occ = np.zeros(len(date_range))

    # Iterate over the date range
    for i, timestamp in enumerate(date_range):
        # Check if it's a weekday (Monday to Friday)
        if timestamp.weekday() < 5:  # Monday=0, Sunday=6
            # Check if the time is between 8 AM (inclusive) and 6 PM (exclusive)
            if 8 <= timestamp.hour < 18:
                full_occ[i] = 1  # Set occupancy to 1 during these hours
...
def ParameterGenerator(
    ...
    Weather: str,
    Location: str,
    Stochastic_Occupancy: bool = True,
    ...
    full_occ: Union[np.ndarray, float] = 1,
    ...
    activities = [
        {'activity': 'Sleeping', 'activity_level_range': (72 * 0.9, 72 * 1.1), 'probability': 1.0},
        {'activity': 'Reclining', 'activity_level_range': (81 * 0.9, 81 * 1.1), 'probability': 0.1},
        {'activity': 'Seated, quiet', 'activity_level_range': (108 * 0.9, 108 * 1.1), 'probability': 0.2},
        {'activity': 'Standing, relaxed', 'activity_level_range': (126 * 0.9, 126 * 1.1), 'probability': 0.1},
        {'activity': 'Reading, seated', 'activity_level_range': (99 * 0.9, 99 * 1.1), 'probability': 0.05},
        {'activity': 'Writing', 'activity_level_range': (108 * 0.9, 108 * 1.1), 'probability': 0.05},
        {'activity': 'Typing', 'activity_level_range': (117 * 0.9, 117 * 1.1), 'probability': 0.05},
        {'activity': 'Filing, seated', 'activity_level_range': (126 * 0.9, 126 * 1.1), 'probability': 0.05},
        {'activity': 'Filing, standing', 'activity_level_range': (144 * 0.9, 144 * 1.1), 'probability': 0.05},
        {'activity': 'Cooking', 'activity_level_range': (171, 207), 'probability': 0.1},  # Already a range
        {'activity': 'House cleaning', 'activity_level_range': (207, 360), 'probability': 0.1}  # Already a range
    ]
    office_occupancy_dic = {
    'MetabolismRange': [70,120]
    }
    ...
    #get the num_timesteps from the length of the groundtemp
    num_timesteps = len(groundtemp)
    ...
    #Extracting the occupancy csv from folder Occupancy and file OfficeSmall.csv
    occupancy_csv = pd.read_csv(f"{root}Occupancy/{Building}/output_byRoom.csv")
    # In columns that start with "S", remove the characters between S and  _ (inclusive)
    if 'Apartment' in Building:
        print('columns in occupancy data:', occupancy_csv.columns)
        
        # Process the occupancy data
        occupancy_csv['Time'] = pd.to_datetime(occupancy_csv['Time'], errors='coerce')
        occupancy_csv = occupancy_csv.resample(f'{time_reso}s', on='Time').max().reset_index()
    
        # Sample occupancy count data from zone_metadata.csv
        zone_metadata = pd.read_csv(f"{root}Occupancy/{Building}/zone_metadata.csv")
        full_occ = zone_metadata['Number of Occupants'].values
    
        # Read and process the schedule data
        schedule_csv = pd.read_csv(f"{root}Occupancy/{Building}/schedule_byRoom.csv")
        print('columns in schedule data:', schedule_csv.columns)
    
        # Ensure 'Time' column is in datetime format
        schedule_csv['Time'] = pd.to_datetime(schedule_csv['Time'], errors='coerce')
    
        schedule_csv.set_index('Time', inplace=True)
        schedule_csv = schedule_csv.resample(f'{time_reso}s').apply(
            lambda x: x.mode()[0] if not x.mode().empty else None  # Return None if no mode found
        ).reset_index()
    
        # Convert schedule data to array-like form (remove the 'Time' column)
        full_schedule = schedule_csv.drop(columns=['Time']).values
    

    elif 'Office' in Building:
        # Add the year '2020' to the "Time" column to make it a complete timestamp
        occupancy_csv['Time'] = '2021 ' + occupancy_csv['Time'].astype(str)

        # Convert the updated "Time" column to datetime format
        occupancy_csv['Time'] = pd.to_datetime(
            occupancy_csv['Time'], 
            format='%Y %m/%d %H:%M:%S', 
            errors='coerce'
        )
        occupancy_csv = occupancy_csv[occupancy_csv['Time'].notna()]
        # Replace column names as needed
        occupancy_csv.columns = [
            col.replace(col[col.find('S'):col.find('_') + 1], '') if col.startswith('S') else col
            for col in occupancy_csv.columns
        ]

        # Set "Time" as the index and resample based on the specified time resolution
        occupancy_csv = occupancy_csv.set_index('Time').resample(f'{time_reso}S').mean()
        print("occupancy_csv head:", occupancy_csv.head())

        full_schedule=None
    stochastic_occupancy = []
    for build in buildall:
        #print(f"Zone: {build[0]}")
        stochastic_occupancy.append(occupancy_csv[build[0]].values)
    
    stochastic_occupancy = np.array(stochastic_occupancy).T
    ...
    if 'Apartment' in Building:
        ...
        nonlinear_stochastic = people_full/np.array([Ctable]).T
    else:
        nonlinear_stochastic = 1/np.array([Ctable]).T
    
    ACweight = np.diag(np.zeros(roomnum)+AC_map) * max_power
    weightcmap = np.concatenate((ground_connectlist,np.zeros((roomnum, 1)), ACweight, np.array([Windowtable*SHGC]).T), axis=-1)/np.array([Ctable]).T
    print("parameters are generated for Stochastic occupancy")

    people_full = np.zeros((roomnum,1))+np.array([full_occ]).T
    ACweight = np.diag(np.zeros(roomnum)+AC_map) * max_power
    weightcmap = np.concatenate((ground_connectlist,np.zeros((roomnum, 1)), ACweight, np.array([Windowtable*SHGC]).T), axis=-1)/np.array([Ctable]).T
    nonlinear_deterministic = people_full/np.array([Ctable]).T
    print("parameters are generated for Deterministic occupancy")
    ...
    Parameter['Stochastic_Occu_Bin'] = stochastic_occupancy
    Parameter['Deterministic_Occu_Bin'] = deterministic_occupancy
    ...
    Parameter['Occupancy'] = activity_sch #older occupancy parameter of the BEAR

    Parameter['ACmap'] = np.zeros(roomnum)+AC_map
    Parameter['max_power'] = max_power
    Parameter['nonlinear_stochastic'] = nonlinear_stochastic
    Parameter['nonlinear_deterministic'] = nonlinear_deterministic
    Parameter['temp_range'] = temp_range
    Parameter['spacetype'] = spacetype

    #parameters for office occupancy
    Parameter['MetabolismRange'] = office_occupancy_dic['MetabolismRange']
    #parameters for residential occupancy
    Parameter['full_occ'] = full_occ #number of occupants in each unit
    Parameter['FullSchedule'] = full_schedule #schedule of events for each unit
    Parameter['activities'] = activities #dictionary containing activity-metabolism rate
