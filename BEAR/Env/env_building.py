"""
The module implements the BuildingEnv class.
"""
from __future__ import annotations

from typing import Any
import functools
import gymnasium as gym
from gymnasium.envs.registration import EnvSpec
import numpy as np
from numpy import linalg as LA
from scipy.linalg import expm
from sklearn import linear_model
import pandas as pd
import random
import os
def sample_activity_level(activities, num_occupants, is_sleeping=False):
    """Sample metabolic rates for all occupants based on whether they are sleeping or doing other activities."""
    if is_sleeping:
        # Use 'Sleeping' activity for all occupants
        sleeping_activity = next(activity for activity in activities if activity['activity'] == 'Sleeping')
        activity_range = sleeping_activity['activity_level_range']
        return [random.uniform(*activity_range) for _ in range(num_occupants)]
    else:
        # Sample from other activities (excluding 'Sleeping')
        non_sleeping_activities = [activity for activity in activities if activity['activity'] != 'Sleeping']
        probabilities = [activity['probability'] for activity in non_sleeping_activities]
        
        # Sample activity levels based on the range
        return [random.uniform(*random.choices(non_sleeping_activities, weights=probabilities, k=1)[0]['activity_level_range']) for _ in range(num_occupants)]

def generate_meta_sch(full_schedule, full_occ, num_timesteps, activities):
    """Generates a metabolic schedule (meta_sch) based on schedule and occupants."""
    
    # Initialize meta_sch as a NumPy array (num_timesteps x number of rooms)
    num_rooms = full_schedule.shape[1]
    meta_sch = np.zeros((num_timesteps, num_rooms))

    # Loop over each room (column in full_schedule) and timestep
    for room in range(num_rooms):
        num_occupants = full_occ[room]  # Get the number of occupants for the room

        for timestep in range(num_timesteps):
            schedule_value = full_schedule[timestep, room]

            if schedule_value == 'Sleep':
                # Sample from 'Sleeping' activity
                metabolic_rates = sample_activity_level(activities, num_occupants, is_sleeping=True)
            else:
                # Sample from other activities
                metabolic_rates = sample_activity_level(activities, num_occupants, is_sleeping=False)

            # Sum the metabolic rates across occupants
            meta_sch[timestep, room] = sum(metabolic_rates)

    return meta_sch


class BuildingEnvReal(gym.Env):
    """BuildingEnvReal class.
    This classes simulates the zonal temperature of a building controlled by a user selected agent.
    It constructs the physics based building simulation model based on the RC model with an
    nonlinear residual model. The simulation is based on the EPW weather file provided by the Building
    Energy Codes Program.

    n = number of zones in the building
    k = number of steps for the MOER CO2 forecast
    Actions:
        Type: Box(n)
        Action                                           Shape       Min         Max
        HVAC power consumption(cool in - ,heat in +)     n           -1          1
    Observations:
        Type: Box(3n+2)
                                                         Shape       Min         Max
        Temperature of zones (celsius)                   n           temp_min    temp_max
        Temperature of outdoor (celsius)                 1           temp_min    temp_max
        Global Horizontal Irradiance (W)                 n           0           heat_max
        Temperature of ground (celsius)                  1           temp_min    temp_max
        Occupancy power (W)                              n           0           heat_max
    Attributes:
        Parameter (dict): Dictionary containing the parameters for the environment.
        observation_space: structure of observations returned by environment
        timestep: current timestep in episode, from 0 to 288
        action_space: structure of actions expected by environment
    """
    metadata = {'render.modes': []}

    # Occupancy nonlinear coefficients, collected from page 1299 of
    # https://energyplus.net/assets/nrel_custom/pdfs/pdfs_v23.1.0/EngineeringReference.pdf
    OCCU_COEF = [
        6.461927,
        0.946892,
        0.0000255737,
        7.139322,
        0.0627909,
        0.0000589172,
        0.19855,
        0.000940018,
        0.00000149532,
    ]

    #Discrete space length
    DISCRETE_LENGTH=100

    #Scaling factor for the reward function weight
    SCALING_FACTOR=24

    def __init__(self, Parameter: Dict[str, Any],user_reward_function=None,reward_breakdown_keys=None):
        """Initializes the environment with the given parameters.
        Args:
            Parameter (dict): Dictionary containing the parameters for the environment.
                'OutTemp' (np.array): Outdoor temperature.
                'connectmap' (np.array): Connection map of the rooms.
                'RCtable' (np.array): RC table of the rooms.
                'roomnum' (int): Number of rooms.
                'weightcmap' (np.array): Weight of the connection map.
                'target' (np.array): Target temperature.
                'gamma' (list): Weight factor for the reward function.
                'ghi' (np.array): Global Horizontal Irradiance.
                'GroundTemp' (np.array): Ground temperature.
                'Occupancy' (np.array): Occupancy of the rooms.
                'ACmap' (np.array): Air conditioning map.
                'max_power' (int): Maximum power for the air conditioning system.
                'nonlinear' (np.array): Nonlinear factor.
                'temp_range' (list): Temperature range (min and max).
                'spacetype' (str): Type of space ('continuous' or 'discrete').
                'time_resolution' (int): Time resolution of the simulation.
        Initializes:
            action_space: Action space for the environment (gym.spaces.Box).
            observation_space: Observation space for the environment (gym.spaces.Box).
            A_d: Discrete-time system matrix A (numpy array).
            B_d: Discrete-time system matrix B (numpy array).
            rewardsum: Cumulative reward in the environment (float).
            statelist: List of states in the environment (list).
            actionlist: List of actions taken in the environment (list).
            epochs: Counter for the number of epochs (int).
        """
        self.Building = Parameter['Building']
        self.OutTemp = Parameter['OutTemp']
        self.length_of_weather=len(self.OutTemp)
        self.connectmap = Parameter['connectmap']
        self.RCtable = Parameter['RCtable']
        self.roomnum = Parameter['roomnum']
        self.weightCmap = Parameter['weightcmap']
        self.target = Parameter['target']
        self.gamma=Parameter['gamma']
        self.ghi=Parameter['ghi']
        self.GroundTemp=Parameter['GroundTemp']
        self.Occupancy=Parameter['Occupancy']
        self.deterministic_occu_bin = Parameter['Deterministic_Occu_Bin']
        self.stochastic_occu_bin=Parameter['Stochastic_Occu_Bin'] #occupancy presence (with counts for office, without counts for residential)
        self.acmap=Parameter['ACmap']
        self.maxpower=Parameter['max_power']
        self.nonlinear_deterministic=Parameter['nonlinear_deterministic']
        self.nonlinear_stochastic=Parameter['nonlinear_stochastic']
        self.temp_range=Parameter['temp_range']
        self.spacetype=Parameter['spacetype']
        self.Occupower=0
        self.timestep = Parameter['time_resolution']
        self.datadriven=False
        self.Stochasticity_Level=0
        #parameter for office occupancy
        self.MetabolismRange=Parameter["MetabolismRange"]
        self.Stochastic_Testing=False
        #parameters for residential occupancy
        self.full_occ=Parameter["full_occ"]
        self.full_schedule=Parameter["FullSchedule"]
        self.activities=Parameter["activities"]

        # Define action space bounds based on room number and air conditioning map
        self.Qlow = np.ones(self.roomnum, dtype=np.float32) * (-1.0)*self.acmap.astype(np.float32)
        self.Qhigh = np.ones(self.roomnum, dtype=np.float32) * (1.0)*self.acmap.astype(np.float32)

        # Set the action space based on the space type
        if self.spacetype == 'continuous':
          self.action_space = gym.spaces.Box(self.Qlow, self.Qhigh, dtype=np.float32)
        else:
          self.action_space = gym.spaces.MultiDiscrete((self.Qhigh*self.DISCRETE_LENGTH-self.Qlow*self.DISCRETE_LENGTH))

        # Set the observation space bounds based on the minimum and maximum temperature
        min_T, max_T = self.temp_range
        heat_max = 1000
        self.low = np.concatenate([
            np.ones(self.roomnum + 1) * min_T,  # temp of zones and outdoor
            np.ones(self.roomnum ) * [0],                          # GHI
            [min_T]   ,                   # temp of ground
            np.ones(self.roomnum ) * [0]  # occupancy power
        ]).astype(np.float32)
        self.high = np.concatenate([
            np.ones(self.roomnum + 1) * max_T,  # temp of zones and outdoor
            np.ones(self.roomnum ) * [heat_max],                     # GHI
            [max_T],                      # temp of ground
            np.ones(self.roomnum ) * [heat_max]                      # occupancy power
        ]).astype(np.float32)
        self.observation_space = gym.spaces.Box(self.low, self.high, dtype=np.float32)

        # Set the weight for the power consumption and comfort range
        self.q_rate = Parameter['gamma'][0]*self.SCALING_FACTOR
        self.error_rate = Parameter['gamma'][1]

        # If a user-defined reward function is provided, use it.
        # Otherwise, use the default reward function.
        if user_reward_function is not None and callable(user_reward_function):
            self.reward_function = functools.partial(user_reward_function, self)
        else:
            self.reward_function = self.default_reward_function


        # If user-defined reward breakdown keys are provided, use them.
        # Otherwise, use the default keys.
        if reward_breakdown_keys is not None and isinstance(reward_breakdown_keys, list):
            self._reward_breakdown = {key: 0.0 for key in reward_breakdown_keys}
        else:
            self._reward_breakdown = {'comfort_level': 0.0, 'power_consumption': 0.0}

        #Track cumulative components of reward
        # self._reward_breakdown = {'comfort_level':0.0,'power_consumption':0.0}

        # Define matrices for the state update equations
        Amatrix = self.RCtable[:, :-1]
        diagvalue = (-self.RCtable) @ self.connectmap.T - np.array([self.weightCmap.T[1]]).T
        np.fill_diagonal(Amatrix, np.diag(diagvalue))
        Bmatrix = self.weightCmap.T
        #print("Initial shape of the B matrix:", Bmatrix.shape)
        Bmatrix[1] = self.connectmap[:, -1] * (self.RCtable[:, -1])
        #print("Modified Bmatrix shape:", Bmatrix.shape)
        Bmatrix = (Bmatrix.T)
        #1 divided by C is given as nonlinear term

        #print length of stochastic occu bin
        print('Length of stochastic occu bin:', len(self.stochastic_occu_bin))
        #print the number of nan values in the stochastic occu bin
        print('Number of nan values in stochastic occu bin:', np.isnan(self.stochastic_occu_bin).sum())
        

        # Initialize reward sum, state list, action list, and epoch counter
        self.rewardsum = 0
        self.statelist = []
        self.actionlist=[]
        self.epochs = 0

        # Initialize zonal temperature
        self.X_new= self.target
        self.Amatrix=Amatrix
        # Compute the discrete-time system matrices
        self.A_d = expm(Amatrix * self.timestep)
        self.B_d = LA.inv(Amatrix) @ (self.A_d - np.eye(self.A_d.shape[0])) @ Bmatrix
        

    def default_reward_function(self, state, action, error, state_new):
        # This is your default reward function
        reward = 0
        reward -= LA.norm(action, 2) * self.q_rate + LA.norm(error, 2) * self.error_rate
        self._reward_breakdown['comfort_level'] -= LA.norm(error, 2) * self.error_rate
        self._reward_breakdown['power_consumption'] -= LA.norm(action, 2) * self.q_rate
        return reward

    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        """Steps the environment.
        Updates the state of the environment based on the given action and calculates the
        reward, done, and info values for the current timestep.
        Args:
            action (np.ndarray): Action to be taken in the environment.
            return_info: info is always returned, but if return_info is set
                to False, only 'zone_temperature' and 'reward_breakdown' are
                returned.
        Returns:
            state (np.ndarray): Updated state of the environment.
                'X_new': shape [roomnum], new temperatures of the rooms.
                'OutTemp': shape [1], outdoor temperature at the current timestep.
                'ghi': shape [1], global horizontal irradiance at the current timestep.
                'GroundTemp': shape [1], ground temperature at the current timestep.
                'Occupower': shape [1], occupancy power at the current timestep.
            reward (float): Reward for the current timestep.
            done (bool): Whether the episode is terminated.
            truncated (bool): Whether the episode has reached a time limit.
            info (dict[str, Any]): Dictionary containing auxiliary information.
                'statelist': List of states in the environment.
                'actionlist': List of actions taken in the environment.
                'epochs': Counter for the number of epochs (int).
        """
        # Scale the action if the space type is not continuous
        if self.spacetype != 'continuous':
          action=(action+self.Qlow*self.DISCRETE_LENGTH)/self.DISCRETE_LENGTH

        # Store the current state in the statelist
        self.statelist.append(self.state)

        # Initialize the 'done' flag as False
        done = False

        # Prepare the input matrices X and Y
        X = self.state[:self.roomnum].T
        Y = np.insert(np.append(action,self.ghi[self.epochs]), 0, self.OutTemp[self.epochs]).T
        Y = np.insert(Y, 0, self.GroundTemp[self.epochs]).T
        #print('Y:', Y)
        avg_temp=np.sum(self.state[:self.roomnum])/self.roomnum
        



        #print('Stochasticity Level:', self.Stochasticity_Level)
        if self.Stochasticity_Level==1:
            index = self.epochs
        elif self.Stochasticity_Level==2:
            time_of_day = self.epochs % self.timesteps_per_day
            
            # Use the random day chosen at reset and compute the global index
            index = (self.day_index + time_of_day) % self.length_of_weather  # Keep the index within valid range
            
            if time_of_day == 23:
                # Move to the next day after the last hour of the current day
                self.day_index = (self.day_index + self.timesteps_per_day) % self.length_of_weather

        if self.Stochasticity_Level>0: 
            
            room_temperatures = self.state[:self.roomnum]
            current_occ_bin = np.array(self.stochastic_occu_bin[index,:])
            
            #print('current_occ_bin:', current_occ_bin, "at index:", index)
            Meta = self.meta_sch[index,:] 
            #print("stochastic meta:", Meta)
            self.Occupower = np.zeros(self.roomnum)
        else:
            Meta = self.Occupancy[self.epochs]
            #print("deterministic meta:", Meta)


        # If the environment is data-driven, add additional features to the Y matrix
        if self.datadriven==True:
          Y = np.insert(Y, 0, Meta).T
          Y = np.insert(Y, 0, Meta**2).T
          Y = np.insert(Y, 0, avg_temp).T
          Y = np.insert(Y, 0, avg_temp**2).T
        else:
          # Calculate Occupower based on the given formula
        
            if self.Stochasticity_Level==0:
                self.Occupower = self._calc_occupower(avg_temp, Meta)
                #print("deterministic Occupower:", self.Occupower, "calculated with avg_temp:", avg_temp, "and Meta:", Meta)

                f = np.full(X.shape, self.Occupower/1000)
            else:
                for i in range(self.roomnum):
                    self.Occupower[i] = self._calc_occupower(room_temperatures[i], Meta[i])
                
                f = current_occ_bin * (self.Occupower/1000) 
                #print("f stochastic:", f)

        #print("f:", f)
        # Update the state using the A_d and B_d matrices

        if self.Stochasticity_Level>0:
            Dmatrix=self.nonlinear_stochastic
        else:
            Dmatrix=self.nonlinear_deterministic

        self.D_d = LA.inv(self.Amatrix) @ (self.A_d - np.eye(self.A_d.shape[0])) @ Dmatrix

        X_new = self.A_d @ X + self.B_d @ Y + self.D_d.flatten() * f #D_d is num_people/C and f is the occupancy power

        # Calculate the error
        error = X_new*self.acmap - self.target*self.acmap

        # Update the reward based on the action and error
        reward = self.reward_function(self.state, action, error, X_new)
        self.rewardsum += reward

        # retrieve environment info
        self.X_new = X_new
        info = self._get_info()

        # Update the state
        ghi_repeated = np.full(X_new.shape, self.ghi[self.epochs])
        
        #print('occ_repeated:', f)
        if self.Stochasticity_Level>0:
            occ_state = current_occ_bin
            print('stochastic occupancy state:', occ_state)
        else:
            occ_state = f = np.full(X.shape, self.full_occ)
            #print("deterministic occupancy state:", occ_state)

        # self.statelist.append(self.state)
        self.state = np.concatenate((X_new, self.OutTemp[self.epochs].reshape(-1,), ghi_repeated, self.GroundTemp[self.epochs].reshape(-1), occ_state), axis=0) #, f

        # Store the action in the actionlist
        self.actionlist.append(action*self.maxpower)

        # Increment the epochs counter
        self.epochs += 1
        

        # Check if the environment has reached the end of the weather data
        if self.epochs>=self.length_of_weather-1:
            done=True
            self.epochs=0

        self.state = np.array(self.state, dtype=np.float32)
        # Return the new state, reward, done flag, and info
        return self.state, reward, done,done,info

    def reset(self, *, seed: int | None = None, options: dict | None = None
              ) -> tuple[np.ndarray, dict[str, Any]]:
        super().reset(seed=seed, options=options)
        """Resets the environment.
        Prepares the environment for the next episode by setting the initial
        temperatures, average temperature, occupancy, and occupower. The initial state
        is constructed by concatenating these variables.

        Args:
            seed: seed for resetting the environment. An episode is entirely
                reproducible no matter the generator used.
            options: resetting options
                'verbose': set verbosity level [0-2]
        Returns:
            state: the initial state of the environment.
            info: information.
        """
        # Initialize the episode counter
        self.epochs = 0

        # Initialize state and action lists
        self.statelist = []
        self.actionlist=[]

        # Use options to get T_initial or use the default value if not provided
        T_initial = self.target if options is None else options.get('T_initial', self.target)

          # T_initial =np.array([18.24489859, 18.58710076, 18.47719682, 19.11476084, 19.59438163,15.39221207])
          # T_initial = np.random.uniform(21,23, self.roomnum+4)

        # Calculate the average initial temperature
        avg_temp=np.sum(T_initial)/self.roomnum

        # If it's an array or list, get the number of timesteps using its shape
        num_timesteps = np.array(self.stochastic_occu_bin).shape[0]


        

        if self.Stochastic_Testing:
            if 'Apartment' in self.Building:
                dir_path = f'deterministic_metabolism_schedule/{self.Building}'
                file_path = os.path.join(dir_path, 'meta_sch.npy')
                if os.path.exists(file_path):
                    # If the .npy file exists, load it into self.meta_sch
                    self.meta_sch = np.load(file_path)
                    print('deterministic metabolism loaded')
                else:
                    # If the directory doesn't exist, create it
                    if not os.path.exists(dir_path):
                        os.makedirs(dir_path)
                    # Generate the metabolism schedule and save it
                    self.meta_sch = generate_meta_sch(
                        self.full_schedule, self.full_occ, num_timesteps, self.activities
                    )
                    # Save the numpy array directly
                    np.save(file_path, self.meta_sch)
                    print("Deterministic meta CREATED")
            elif 'Office' in self.Building:
                dir_path = f'deterministic_metabolism_schedule/{self.Building}'
                file_path = os.path.join(dir_path, 'meta_sch.npy')
                if os.path.exists(file_path):
                    # If the .npy file exists, load it into self.meta_sch
                    self.meta_sch = np.load(file_path)
                    print('stochastic metabolism for testing has been loaded')
                else:
                    # If the directory doesn't exist, create it
                    if not os.path.exists(dir_path):
                        os.makedirs(dir_path)
                    # Generate random metabolism values and save them
                    meta_low, meta_up = self.MetabolismRange
                    self.meta_sch = np.random.uniform(
                        meta_low, meta_up, size=(num_timesteps, self.roomnum)
                    )
                    # Save the numpy array directly
                    np.save(file_path, self.meta_sch)
                    print("stochastic meta CREATED")
        else:
            if 'Apartment' in self.Building:
                self.meta_sch = generate_meta_sch(self.full_schedule, self.full_occ, num_timesteps, self.activities)
                #print("Generated meta_sch (preview):", self.meta_sch[:5, :5])  # Print the first 5 rows and 5 columns as a preview
            
            elif 'Office' in self.Building:
                meta_low, meta_up = self.MetabolismRange
                    
                self.meta_sch = np.random.uniform(meta_low, meta_up, size=(num_timesteps, self.roomnum))

        if self.Stochasticity_Level>0:
            room_temperatures = T_initial
            Metabolism_Array = self.meta_sch[self.epochs]  # Meta data for occupancy at this epoch
            #print("Meta",np.shape(Metabolism_Array))
            self.Occupower = np.zeros(self.roomnum)  # Initialize Occupower array

            for i in range(self.roomnum):
                self.Occupower[i] = self._calc_occupower(room_temperatures[i], Metabolism_Array[i])
                
            self.timesteps_per_day = int(24 * 3600 / self.timestep)

        if self.Stochasticity_Level==2:
            print('Stochasticity Level 2')
            #compute the number of days available in occupancy array by dividing it with timesteps_per_day
            days_available = self.stochastic_occu_bin.shape[0] / self.timesteps_per_day
            #print('days available:', days_available)
            #randomly choose a day from the available days and compute the index of the day in the occupancy array
            self.random_day = np.random.randint(0, days_available)
            #print('random day:', self.random_day)
            self.day_index = self.random_day * self.timesteps_per_day
            initial_occ_bin = np.array(self.stochastic_occu_bin[self.day_index]) 
            occ_repeated = initial_occ_bin * (self.Occupower / 1000)
            occ_state = initial_occ_bin
        elif self.Stochasticity_Level==1:
            print('Stochasticity Level 1')
            initial_occ_bin = np.array(self.stochastic_occu_bin[0,:]) 
            #print('initial_occ_bin:', initial_occ_bin)
            occ_repeated = initial_occ_bin * (self.Occupower / 1000)
            occ_state = initial_occ_bin
        elif self.Stochasticity_Level==0:
            print('Deterministic')
            # Get the occupancy value for the current epoch
            Meta = self.Occupancy[self.epochs]

            # Calculate the occupower based on occupancy and average temperature
            self.Occupower = self._calc_occupower(avg_temp, Meta)

            self.X_new=T_initial
            occ_repeated= np.full(T_initial.shape, self.Occupower/1000)
            occ_state=np.full(T_initial.shape, self.full_occ)
        
        #print('occ_repeated at reset:', occ_repeated)
        # Construct the initial state by concatenating relevant variables
        self.X_new=T_initial
        ghi_repeated = np.full(T_initial.shape, self.ghi[self.epochs])
        self.state = np.concatenate((T_initial,self.OutTemp[self.epochs].reshape(-1,), ghi_repeated, self.GroundTemp[self.epochs].reshape(-1), occ_state), axis=0) #, occ_repeated

        # Initialize the rewards
        self.flag=1
        self.rewardsum = 0
        for re in self._reward_breakdown:
          self._reward_breakdown[re] = 0.0
        # print("Reset", self.state)
        self.state = np.array(self.state, dtype=np.float32)
        # Return the initial state and an empty dictionary(for gymnasium grammar)
        return self.state, self._get_info()
    def _get_info(self, all: bool = False) -> Dict[str, Any]:
        """
        Returns info. See step().

        Args:
            all: whether all information should be returned. Otherwise, only
                'zone_temperature' and 'reward_breakdown' are returned.
        """
        if all:
            return {
                'zone_temperature': self.X_new,
                'out_temperature': self.OutTemp[self.epochs].reshape(-1,),
                'ghi': self.ghi[self.epochs].reshape(-1),
                'ground_temperature': self.GroundTemp[self.epochs].reshape(-1),
                'reward_breakdown': self._reward_breakdown,
            }
        else:
            return {
                'zone_temperature': self.X_new,
                'reward_breakdown': self._reward_breakdown,
            }
        
    def _calc_occupower(self, avg_temp: float, Meta: float) -> float:
        return (
            self.OCCU_COEF[0]
            + self.OCCU_COEF[1] * Meta
            + self.OCCU_COEF[2] * Meta**2
            + self.OCCU_COEF[3] * avg_temp
            - self.OCCU_COEF[4] * avg_temp * Meta
            + self.OCCU_COEF[5] * avg_temp * Meta**2
            - self.OCCU_COEF[6] * avg_temp**2
            + self.OCCU_COEF[7] * avg_temp**2 * Meta
            - self.OCCU_COEF[8] * avg_temp**2 * Meta**2
        )
            
    def train(self, states: np.ndarray, actions: np.ndarray) -> None:
        """Trains the linear regression model using the given states and actions.
        The model is trained to predict the next state based on the current state and action.
        The trained coefficients are stored in the environment for later use.

        Args:
            states: a list of states.
            actions: a list of actions corresponding to each state.
        """
        # Initialize lists to store current states and next states
        current_state=[]
        next_state=[]

        # Iterate through states and actions to create input and output data for the model
        for i in range(len(states)-1):
            X=states[i]
            Y = np.insert(np.append(actions[i]/self.maxpower,self.ghi[i]), 0, self.OutTemp[i]).T
            Y = np.insert(Y, 0, self.GroundTemp[i]).T
            avg_temp=np.sum(X)/self.roomnum
            Meta = self.Occupancy[i]

          # Calculate the occupower based on occupancy and average temperature
            self.Occupower = self._calc_occupower(avg_temp, Meta)

            # Add relevant variables to Y
            Y = np.insert(Y, 0, Meta).T
            Y = np.insert(Y, 0, Meta**2).T
            Y = np.insert(Y, 0, avg_temp).T
            Y = np.insert(Y, 0, avg_temp**2).T

            # Concatenate X and Y to form the input data for the model
            stackxy=np.concatenate((X,Y), axis=0)

            # Append the input data and next state to their respective lists
            current_state.append(stackxy)
            next_state.append(states[i+1])

        # Create a linear regression model with non-negative coefficients and no intercept
        model = linear_model.LinearRegression(fit_intercept=False,positive=True)

        # Fit the model using the input and output data
        modelfit = model.fit(np.array(current_state),np.array(next_state))

        # Get the coefficients of the fitted model
        beta = modelfit.coef_

        # Update the A_d and B_d matrices with the coefficients from the fitted model
        self.A_d=beta[:,:self.roomnum]
        self.B_d=beta[:,self.roomnum:]

        # Set the data-driven flag to True
        self.datadriven=True
        # return current_state,next_state

    def render(self, mode: str = 'human') -> None:
        pass

    def close(self) -> None:
        pass
